"""Validate extracted GeoParquet output locally (supports Hive-partitioned layout)."""
...
# Row count check
r = db.execute(f"SELECT COUNT(*) AS n FROM read_parquet('{PATH}', hive_partitioning = true)").fetchone()
count = r[0]
...
cols = db.execute(f"DESCRIBE SELECT * FROM read_parquet('{PATH}', hive_partitioning = true)").fetchall()
